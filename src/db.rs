...
pub async fn run_migrations(pool: &PgPool) {
    ...
    sqlx::raw_sql(include_str!("../migrations/003_update_web_contexts.sql"))
        .execute(pool)
        .await
        .expect("Failed to run migration 003");

    sqlx::raw_sql(include_str!("../migrations/004_fraud_v2.sql"))
        .execute(pool)
        .await
    ...
    sqlx::raw_sql(include_str!("../migrations/006_drop_discord_tables.sql"))
        .execute(pool)
        .await
        .expect("Failed to run migration 006");

    sqlx::raw_sql(include_str!("../migrations/007_guild_settings.sql"))
        .execute(pool)
        .await
