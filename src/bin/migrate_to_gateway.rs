...
//!
//! Skips `oauth_states` entirely (schemas differ and rows are short-lived
//! CSRF state — not worth migrating).
//!
//! Usage (PowerShell):
//!   $env:DATABASE_URL="postgres://app:password@localhost:5432/member_origin_role"
...
async fn main() {
    ...
    let plugin_url =
        env::var("DATABASE_URL").expect("DATABASE_URL (Member-Origin plugin DB) must be set");
    let gateway_url = env::var("GATEWAY_DATABASE_URL")
        .expect("GATEWAY_DATABASE_URL (Auth Gateway DB) must be set");

    println!("Connecting to source (Member-Origin plugin)...");
    let src = PgPoolOptions::new()
        .max_connections(4)
        .acquire_timeout(std::time::Duration::from_secs(10))
        .connect(&plugin_url)
        .await
        .expect("failed to connect to Member-Origin plugin DB");
    ...
    println!("\nDone. oauth_states was intentionally skipped (ephemeral CSRF state).");
}
...
async fn migrate_discord_tokens(src: &PgPool, dst: &PgPool) {
    ...
    let rows = sqlx::query(
        "SELECT discord_id, refresh_token, guilds_refreshed_at, created_at \
         FROM discord_tokens",
    )
    .fetch_all(src)
    .await
